...
VOTETYPE_DATA_FILEPATH = (
    f"{INPUT_DIRECTORY}{os.sep}HouseTppByDivisionByVoteTypeDownload-27966.csv"
)
VOTETYPE_DATA_OUTPUT_FILEPATH = f"{OUTPUT_DIRECTORY}{os.sep}f2022_tpp_by_vote_type.csv"
...
df_pc_total.columns = [
    (
        f"tpp_{col.lower()}_f2022_pc_tot"
...
df_primary_results_totals.rename(
    columns={"TotalVotes": "tpp_f2022_tot"},
    inplace=True,
)
...
df_output.to_csv(
    f"{OUTPUT_DIRECTORY}{os.sep}f2022_tpp_by_vote_type_pc.csv",
    encoding="UTF8",
    index=False,
)
...
df_pc_total.to_csv(
    f"{OUTPUT_DIRECTORY}{os.sep}f2022_tpp_by_vote_type_pc_total.csv", encoding="UTF8"
)
