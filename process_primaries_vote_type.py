...
YEAR = ensure_env_var_exists("YEAR")
...
PRIMARY_VOTETYPE_DATA_FILEPATH = (
    f"{INPUT_DIRECTORY}{os.sep}HouseFirstPrefsByCandidateByVoteTypeDownload-{ELECTION_CODE}.csv"
)

PRIMARY_VOTETYPE_DATA_OUTPUT_FILEPATH = (
    f"{OUTPUT_DIRECTORY}{os.sep}f{YEAR}_fp_by_vote_type.csv"
)
...
df_pc_total.columns = [
    (
        f"fp_{col.lower()}_f{YEAR}_pc_tot"
...
df_pc_total_cutdown = df_pc_total.reset_index()[
    ["DivisionNm", "VoteType", "fp_inf_f{YEAR}_pc_tot"]
]
...
df_primary_results_totals.rename(
    columns={"TotalVotes": "fp_f{YEAR}_tot", "TotalFormalVotes": "fp_f{YEAR}_tot_formal"},
    inplace=True,
)
...
df_output.to_csv(
    f"{OUTPUT_DIRECTORY}{os.sep}f{YEAR}_fp_by_vote_type_pc.csv",
    encoding="UTF8",
    index=False,
)
...
df_pc_formal.to_csv(
    f"{OUTPUT_DIRECTORY}{os.sep}f{YEAR}_fp_by_vote_type_pc_formal.csv",
    encoding="UTF8",
)

df_pc_total = df_pc_total.round(4)
df_pc_total.to_csv(
    f"{OUTPUT_DIRECTORY}{os.sep}f{YEAR}_fp_by_vote_type_pc_total.csv", encoding="UTF8"
)
